    ...
    return (
      <div className="bg-slate-900/90 border border-cyan-500/30 p-3 rounded-lg shadow-xl backdrop-blur-sm z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          {data.hasFlowBuff && (<span className="ml-2 text-[10px] bg-orange-500/20 text-orange-400 px-1.5 py-0.5 rounded border border-orange-500/50 flex items-center gap-1"><Flame size={8} /> 流年加持</span>)}
        </div>
      </div>
    );
  }
...
const FlowTriangle = ({ flowValue, mainValue, colorClass, scale = 1 }) => {
  ...
  return (
    <div className="relative w-28 h-24 ml-4 shrink-0 select-none" style={{ transform: `scale(${scale})`, transformOrigin: 'center center' }}>
...
const SoulMatrix = ({ data, colorClass }) => {
  ...
  return (
    <div className="relative flex items-center justify-center shrink-0" style={{ width: WIDTH, height: HEIGHT }}>
      ...
      {gridNums.map((num) => {
          ...
          return (
            <div key={num} className="absolute w-14 h-14 flex items-center justify-center transform -translate-x-1/2 -translate-y-1/2" style={{ left: pos.x, top: pos.y }}>
              ...
              <div className={`w-10 h-10 rounded md:rounded-lg flex items-center justify-center font-mono text-xl font-bold z-10 relative overflow-hidden border ${hasNum ? `bg-${themeTailwind}-900 text-white border-${themeTailwind}-400` : 'bg-slate-900 text-slate-700 border-slate-800'}`}>{num}</div>
              {count > 1 && (<div className={`absolute -top-2 -right-2 w-5 h-5 rounded-full bg-slate-950 border border-${themeTailwind}-500 text-${themeTailwind}-400 text-[10px] flex items-center justify-center font-bold z-20 shadow-lg`}>x{count}</div>)}
            </div>
          );
      })}
      ...
      <div className="absolute -bottom-6 w-full flex flex-wrap justify-center gap-2 pointer-events-none">
          {lines.map(line => (
              <span key={line.id} className={`text-[10px] px-2 py-0.5 rounded border border-${themeTailwind}-500/30 bg-${themeTailwind}-950/50 text-${themeTailwind}-400`}>{line.name}</span>
          ))}
      </div>
    </div>
  );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-[#050B14]/95 backdrop-blur-xl flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-7xl flex justify-between items-center mb-4 border-b border-slate-800 pb-4 shrink-0">
        <div className="flex items-center gap-3"><Database className="text-cyan-400" size={24} /><h2 className="text-2xl font-bold text-white">生命基礎軌跡</h2></div>
        <div className="flex gap-2"><div className="flex bg-slate-900 rounded-lg p-1 border border-slate-700"><button onClick={() => setActiveTab('timeline')} className={`px-4 py-2 rounded text-sm font-bold ${activeTab === 'timeline' ? 'bg-cyan-600 text-white' : 'text-slate-400'}`}>時間軌跡</button><button onClick={() => setActiveTab('matrix')} className={`px-4 py-2 rounded text-sm font-bold ${activeTab === 'matrix' ? 'bg-purple-600 text-white' : 'text-slate-400'}`}>靈魂晶片</button></div><button onClick={onClose}><X size={28} className="text-slate-400 hover:text-white" /></button></div>
      </div>
      
      {loading ? <Activity className="animate-spin text-cyan-400" /> : (
        <div className="w-full max-w-7xl h-[85vh] overflow-hidden flex flex-col">
          
          {activeTab === 'timeline' && (
            <div className="flex flex-col gap-6 h-full">
               <div className="flex items-center justify-center gap-6 shrink-0"><button onClick={()=>setCycleIndex(Math.max(0, cycleIndex-1))}><ChevronLeft className="text-white" size={32}/></button><h3 className="text-3xl font-bold text-white font-mono">CYCLE {cycleIndex+1}</h3><button onClick={()=>setCycleIndex(Math.min(10, cycleIndex+1))}><ChevronRight className="text-white" size={32}/></button></div>
                  ...
                  {currentCycleData.map((item, idx) => (
                      <div key={idx} className={`border ${item.year === new Date().getFullYear() ? 'border-cyan-500 bg-slate-900' : 'border-slate-800 bg-slate-900/50'} rounded-2xl p-6 relative`}>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <div className="bg-black/30 p-3 rounded border border-slate-700/50"><div className="text-xs text-cyan-500 mb-1">陽曆</div><div className="text-2xl text-white font-mono font-bold">{formatRawNum(item.solarFlow)}</div></div>
                              <div className="bg-black/30 p-3 rounded border border-slate-700/50"><div className="text-xs text-purple-500 mb-1">陰曆</div><div className="text-2xl text-white font-mono font-bold">{formatRawNum(item.lunarFlow)}</div></div>
                          </div>
                          <div className="mt-4 text-center text-sm text-slate-400 bg-slate-950/50 py-1 rounded border border-slate-800">{item.insight}</div>
                      </div>
                  ))}
               </div>
            </div>
          ...
          {activeTab === 'matrix' && matrixData && (
             <div className="flex w-full h-full items-center justify-center gap-10">
                 <div className="relative w-[340px] h-[450px] bg-slate-900/50 border border-slate-800 rounded-2xl flex flex-col items-center justify-center">
                     <div className="absolute top-6 left-6 flex gap-2 text-cyan-400 font-bold text-xl"><Sun/> 陽曆晶片</div>
                     <SoulMatrix data={matrixData.solar} colorClass="cyan"/>
                 </div>
                 <div className="relative w-[340px] h-[450px] bg-slate-900/50 border border-slate-800 rounded-2xl flex flex-col items-center justify-center">
                     <div className="absolute top-6 left-6 flex gap-2 text-purple-400 font-bold text-xl"><Moon/> 陰曆晶片</div>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-[#050B14]/95 backdrop-blur-xl flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-4xl border-b border-slate-800 pb-4 mb-4 flex justify-between items-center">
                <div className="flex items-center gap-3"><Compass className="text-green-400"/><h2 className="text-2xl font-bold text-white">流年戰術導航</h2></div>
            ...
            <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pb-10">
                <div className="bg-slate-900/50 border border-cyan-900/50 rounded-xl p-6 relative overflow-hidden">
                    <h3 className="text-cyan-400 font-bold mb-4 flex items-center gap-2"><Sun size={18}/> 陽曆 (外在顯化)</h3>
                    <div className="space-y-4 relative z-10">
                        <div className="flex justify-between bg-black/40 p-3 rounded-lg border border-slate-800"><span className="text-slate-400 text-xs">流年</span><div className="text-2xl font-bold text-white">{formatRawNum(data.solar)}</div></div>
                        <div className="flex justify-between bg-black/40 p-3 rounded-lg border border-slate-800"><span className="text-slate-400 text-xs">流月</span><div className="text-2xl font-bold text-white">{formatRawNum(data.solarMonth)}</div></div>
                        <div className="flex justify-between bg-cyan-950/30 p-3 rounded-lg border border-cyan-500/50"><span className="text-cyan-300 text-xs font-bold flex items-center gap-1"><Flame size={12}/> 今日流日</span><div className="text-3xl font-bold text-white animate-pulse">{formatRawNum(data.solarDay)}</div></div>
                    </div>
                </div>
                <div className="bg-slate-900/50 border border-purple-900/50 rounded-xl p-6 relative overflow-hidden">
                    <h3 className="text-purple-400 font-bold mb-4 flex items-center gap-2"><Moon size={18}/> 陰曆 (內在潛影)</h3>
                    <div className="space-y-4 relative z-10">
                        <div className="flex justify-between bg-black/40 p-3 rounded-lg border border-slate-800"><span className="text-slate-400 text-xs">流年</span><div className="text-2xl font-bold text-white">{formatRawNum(data.lunar)}</div></div>
                        <div className="flex justify-between bg-black/40 p-3 rounded-lg border border-slate-800"><span className="text-slate-400 text-xs">流月</span><div className="text-2xl font-bold text-white">{formatRawNum(data.lunarMonth)}</div></div>
                        <div className="flex justify-between bg-purple-950/30 p-3 rounded-lg border border-purple-500/50"><span className="text-purple-300 text-xs font-bold flex items-center gap-1"><Flame size={12}/> 今日流日</span><div className="text-3xl font-bold text-white animate-pulse">{formatRawNum(data.lunarDay)}</div></div>
                    </div>
                </div>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-[#050B14]/95 backdrop-blur-xl flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-5xl border-b border-slate-800 pb-4 mb-4 flex justify-between items-center shrink-0">
                ...
                <div className="flex items-center gap-3"><Waves className="text-fuchsia-500"/><h2 className="text-2xl font-bold text-white">能量共振實驗室 v10.9</h2></div>
                <button onClick={onClose}><X size={24} className="text-slate-400 hover:text-white" /></button>
            </div>
            ...
            <div className="w-full max-w-5xl h-full flex gap-4 items-stretch overflow-hidden pb-4">
                <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 flex flex-col items-center justify-center w-1/4 shrink-0">
                    ...
                    <div className="text-4xl font-bold text-cyan-400 font-mono mb-2">{formatRawNum(myData.mainSolar)}</div>
                    <div className="text-2xl font-bold text-purple-400 font-mono mb-6">{formatRawNum(myData.mainLunar)}</div>
                    <div className="opacity-50"><User size={64} className="text-slate-600"/></div>
                </div>
                ...
                <div className="flex-1 flex items-center justify-center relative">
                    {!partner ? (
                        ...
                        <div className="grid grid-cols-3 grid-rows-3 gap-4 w-full max-w-md aspect-square relative z-10 p-4">
                            <div className="col-start-2 row-start-1 bg-gradient-to-b from-fuchsia-950/50 to-slate-900/50 border border-fuchsia-500/50 rounded-xl flex flex-col items-center justify-center p-2 shadow-[0_0_15px_rgba(232,79,255,0.2)]">
                            ...
                            <div className="col-start-1 row-start-2 bg-gradient-to-r from-cyan-950/50 to-slate-900/50 border border-cyan-500/50 rounded-xl flex flex-col items-center justify-center p-2">
                            ...
                            <div className="col-start-2 row-start-2 z-20 flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-md border-2 border-slate-600 rounded-full shadow-[0_0_30px_rgba(255,255,255,0.05)] aspect-square">
                                 ...
                                 <Waves size={24} className="text-fuchsia-400 animate-pulse my-1"/>
                            ...
                            <div className="col-start-3 row-start-2 bg-gradient-to-l from-purple-950/50 to-slate-900/50 border border-purple-500/50 rounded-xl flex flex-col items-center justify-center p-2">
                            ...
                            <div className="col-start-2 row-start-3 bg-gradient-to-t from-purple-950/50 to-slate-900/50 border border-purple-500/50 rounded-xl flex flex-col items-center justify-center p-2">
                ...
                <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 flex flex-col items-center relative w-1/4 shrink-0">
                    {partner ? (
                        <div className="w-full h-full flex flex-col items-center justify-center">
                            ...
                            <div className="text-4xl font-bold text-cyan-400 font-mono mb-2">{formatRawNum(partner.mainSolar)}</div>
                            <div className="text-2xl font-bold text-purple-400 font-mono mb-6">{formatRawNum(partner.mainLunar)}</div>
                            <button onClick={() => setPartner(null)} className="absolute top-4 right-4 text-slate-600 hover:text-red-400 p-1 bg-slate-800 rounded-full"><X size={20}/></button>
                            <User size={64} className="text-slate-600 opacity-50"/>
                        </div>
                    ) : (
                                ...
                                {history.filter(h => h.name !== myData.name).length === 0 ? (
                                    ...
                                    history.filter(h => h.name !== myData.name).map(h => (
                                        <button key={h.id} onClick={() => { onCalculate(h.birthday).then(res => { setPartner({ ...h, mainSolar: res.mainSolar, mainLunar: res.mainLunar, mainSolarNum: res.mainSolarNum, mainLunarNum: res.mainLunarNum }); }); }} className="p-3 bg-slate-800/50 border border-slate-700 hover:bg-cyan-900/30 hover:border-cyan-500/30 rounded-lg text-sm text-slate-300 text-left truncate transition flex justify-between items-center group shrink-0">
                                            ...
                                            <ChevronRight size={16} className="opacity-0 group-hover:opacity-100 text-cyan-400 transition-opacity"/>
                                        </button>
                                    ))
                                )}
                            </div>
...
export default function SoulDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#050B14] text-slate-200 font-sans selection:bg-cyan-500/30 overflow-x-hidden">
      ...
      <AnimatePresence>{showInputModal && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#050B14]/80 backdrop-blur-md"><div className="bg-slate-900 border border-cyan-500/30 p-8 rounded-2xl shadow-2xl shadow-cyan-500/20 max-w-md w-[95%] flex flex-col"><div className="text-center mb-6"><h2 className="text-2xl font-bold">靈魂藍圖掃描</h2></div><div className="space-y-4 mb-6"><input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="姓名" className="w-full bg-slate-950 border border-slate-700 rounded px-4 py-2 text-center" /><div className="grid grid-cols-3 gap-2"><input type="number" value={birthdayInput.year} onChange={(e) => setBirthdayInput({...birthdayInput, year: parseInt(e.target.value)})} className="bg-slate-950 border border-slate-700 rounded px-2 py-2 text-center"/><input type="number" value={birthdayInput.month} onChange={(e) => setBirthdayInput({...birthdayInput, month: parseInt(e.target.value)})} className="bg-slate-950 border border-slate-700 rounded px-2 py-2 text-center"/><input type="number" value={birthdayInput.day} onChange={(e) => setBirthdayInput({...birthdayInput, day: parseInt(e.target.value)})} className="bg-slate-950 border border-slate-700 rounded px-2 py-2 text-center"/></div></div><button onClick={handleStartScan} className="w-full bg-cyan-600 p-3 rounded font-bold text-white">啟動分析</button><div className="mt-4 border-t border-slate-800 pt-2 h-32 overflow-y-auto custom-scrollbar">{history.map(h => <div key={h.id} onClick={()=>{setUserName(h.name);setBirthdayInput(h.birthday)}} className="p-2 hover:bg-slate-800 cursor-pointer flex justify-between"><span>{h.name}</span><span>{h.dateStr}</span></div>)}</div></div></motion.div>)}</AnimatePresence>
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-slate-800 bg-[#050B14]/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 text-cyan-400"><LayoutDashboard size={20} /><span className="font-bold tracking-widest text-sm">{userName ? `${userName} 的靈魂導航儀` : '全息靈魂導航儀 v10.9'}</span></div>
          <div className="flex items-center gap-6 bg-slate-900/50 px-6 py-2 rounded-full border border-slate-700/50"><button onClick={() => setYear(y => y - 1)}><ChevronLeft size={20} /></button><span className="text-2xl font-mono font-bold text-white flex items-center gap-2">{year}{loading && <Activity size={14} className="animate-spin text-cyan-500"/>}</span><button onClick={() => setYear(y => y + 1)}><ChevronRight size={20} /></button></div>
          <div className="flex items-center gap-2">
              <button onClick={() => setShowTactical(true)} className="p-2 bg-slate-800 hover:bg-green-900 rounded-full text-slate-400 hover:text-green-400 transition" title="流年戰術"><Compass size={18} /></button>
              <button onClick={() => setShowResonance(true)} className="p-2 bg-slate-800 hover:bg-pink-900 rounded-full text-slate-400 hover:text-pink-400 transition" title="能量共振"><Waves size={18} /></button>
              <button onClick={handleSnapshot} className="p-2 bg-slate-800 hover:bg-cyan-900 rounded-full text-slate-400 hover:text-cyan-400 transition" title="截圖"><Camera size={18} /></button>
              <button onClick={() => setShowInputModal(true)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 hover:text-white transition" title="設定"><Settings size={18} /></button>
          </div>
        </div>
      </nav>
        ...
        <div className="relative flex flex-col items-center justify-center py-10">
          ...
          <motion.div className="relative z-10 flex flex-col items-center gap-8 w-full">
            <div className="w-full max-w-[280px] bg-slate-900/80 border border-slate-700/50 rounded-lg p-3 backdrop-blur-md flex flex-col items-center gap-2 shadow-lg shadow-cyan-900/10 mb-4">
                <div className="flex items-center gap-2 text-xs text-slate-400 mb-1 border-b border-slate-700/50 w-full justify-center pb-1"><Calendar size={12} /> 生日資訊</div>
                <div className="w-full grid grid-cols-[40px_1fr] gap-2 px-2 items-center"><span className="text-[10px] text-cyan-500/70 text-left">陽曆</span><span className="text-sm font-bold text-white font-mono text-right">{cleanDateStr(data.solarDateStr)}</span></div>
                <div className="w-full grid grid-cols-[40px_1fr] gap-2 px-2 items-center"><span className="text-[10px] text-purple-500/70 text-left">陰曆</span><span className="text-sm font-bold text-slate-300 font-mono text-right">{cleanDateStr(data.lunarDateStr)}</span></div>
            </div>
            <div className="relative group cursor-pointer" onClick={() => setShowInputModal(true)}>
              ...
              <div className="w-60 h-60 bg-[#050B14] border-2 border-slate-700 rounded-full flex flex-col items-center justify-center relative shadow-2xl shadow-cyan-900/20 px-4">
                <div className="text-xs text-slate-500 tracking-widest mb-4">主命數</div>
                <div className="flex items-center justify-between w-full px-4 mb-2"><span className="text-3xl font-bold text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]">{formatRawNum(data.mainSolar)}</span><motion.div className="flex items-center gap-1" animate={{ scale: [1, 1.1, 1], opacity: [0.8, 1, 0.8] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}><Atom size={18} className="text-cyan-500/80" /><span className="text-xl font-bold text-cyan-300">{data.mainSolarLv}</span></motion.div></div>
                <div className="w-full h-[1px] bg-slate-800 my-3"></div>
                <div className="flex items-center justify-between w-full px-4 mt-2"><span className="text-3xl font-bold text-purple-400 drop-shadow-[0_0_10px_rgba(192,132,252,0.5)]">{formatRawNum(data.mainLunar)}</span><motion.div className="flex items-center gap-1" animate={{ scale: [1, 1.1, 1], opacity: [0.8, 1, 0.8] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 1.5 }}><Atom size={18} className="text-purple-500/80" /><span className="text-xl font-bold text-purple-300">{data.mainLunarLv}</span></motion.div></div>
              </div>
            </div>
            <div className="flex flex-col gap-3 w-full px-4">
              <button onClick={() => setShowOverview(true)} className="w-full px-6 py-3 bg-gradient-to-r from-slate-900 to-slate-800 border border-slate-600 hover:border-cyan-400 hover:shadow-[0_0_15px_rgba(34,211,238,0.2)] text-white rounded-lg text-sm tracking-widest transition-all flex items-center justify-center gap-2 group"><Grid size={16} className="text-cyan-400 group-hover:text-white transition" />開啟全息總覽</button>
              <button onClick={() => setShowInputModal(true)} className="w-full px-6 py-2 border border-slate-800 hover:border-slate-600 hover:bg-slate-900/50 text-slate-400 hover:text-white rounded-lg text-xs tracking-widest transition-all flex items-center justify-center gap-2"><ScanFace size={14} />切換使用者 / 重新掃描</button>
            </div>
          </motion.div>
        </div>
